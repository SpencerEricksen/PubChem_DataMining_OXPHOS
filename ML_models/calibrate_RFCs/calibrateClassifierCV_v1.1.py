...
try:
    ...
    print('usage:    python calibrate.py   feat_type   in_model_name   out_model_name')
    sys.exit(1)
...
model_filename = "_".join( model_name.split('/')[-1].split('.')[0:-1] ) + "_calibrated.pkl"
pickle.dump( calibrated_rfc, open( model_filename, 'wb'))

# predict probabilies w calibrated model (prob for positive outcome in second column)
probs = calibrated_rfc.predict_proba(X)[:,1]

# keep class predictions using calibrated model on calibration data (default threshold)
