...
# looks like an outlier I want to remove
...
#desc_cols = [ i for i in df.columns if 'desc_' in i ]
...
for c in df_Y.columns:
    ...
    if (df_Y[c].sum() == 79) or (df_Y[c].sum() == 0):
...
def get_models():
    ...
    # n_trees
    for i in [ 200, 400, 800, 1600, 2000 ]:
