...
df2.drop( columns=[ i for i in df2.columns[2:] if 'fr_' in i ], inplace=True )
...
df_diff.rename( columns={'index':'prop'}, inplace=True )
#prop_list = [ i[5:] for i in df_diff['prop'].tolist() ]

# dump property difference data to CSV
df_diff.to_csv('diff_data_desc_props_mean_st.csv', index=False )

# get only the most divergent features for plotting
df_diff2 = pd.concat( [ df_diff.iloc[0:20], df_diff.iloc[-20:] ] )
prop_list = [ i[5:] for i in df_diff2['prop'].tolist() ]
...
# Decorations
plt.yticks( df_diff2.index.tolist(), prop_list, fontsize=12)
#plt.title('Feature Div, Mean/StandardScaler', fontdict={'size':12} )
plt.title('Relative Property Values in Actives', fontdict={'size':12} )
plt.grid(linestyle='--', alpha=0.5)
plt.xlim(-1.25, 1.25)
plt.savefig( 'Fig2A_diff_bars_props_'+scaler+'_'+stat+'.png', bbox_inches='tight', dpi=600 )
