    ...
    for c in [0.001, 0.01, 0.1, 1.0, 10.]:
        ...
        for k in ['sigmoid']:
            ...
            for g in [ 'scale', 'auto', 0.1, 0.5 ]:
                key_g = str(g)
                # coef0 --> extra kernel term in poly and sigmoid
                for theta in [ -1.0, 0.0, 0.3, 1.0, 3.0 ]:
                    key_theta = '%.1f' % theta
                    key = str(key_c) + "_" + str(key_k) + "_degree3_" + str(key_g) + "_" + str(key_theta) + feat_type
                    models[key] = SVC( C=c, kernel=k, gamma=g, degree=3, coef0=theta, class_weight='balanced', probability=True )
    return models

# get the models to evaluate
models = get_models()

f = open('svc_opt1_fps_sigmoid_stdscale_loggers.txt','w')
