...
if feat_type == 'desc':
    ...
    # no need to scale since Tanimoto kernel probably likes bit vectors
...
def get_models():
    ...
    models = dict()
    # reg param
    # 0.010_tanimoto_nogamma_fps,0.840,0.013,0.441,0.050,0.912,0.015,0.170,0.007
    # 0.100_tanimoto_nogamma_fps,0.846,0.009,0.648,0.046,0.942,0.014,0.569,0.021
    # 1.000_tanimoto_nogamma_fps,0.776,0.019,0.723,0.039,0.936,0.014,0.692,0.041
    # 10.000_tanimoto_nogamma_fps,0.768,0.021,0.726,0.040,0.935,0.014,0.684,0.041

    for c in [ 0.5, 2.0, 5.0, 15.0, 30.0]:
        key_c = '%.3f' % c
        ...
        for k in [ Tanimoto ]:
            ...
            key = str(key_c) + "_" + str(key_k) + "_nogamma_" + feat_type
            models[key] = SVC( C=c, kernel=Tanimoto(), class_weight='balanced', probability=True )
    return models

# get the models to evaluate
models = get_models()

f = open('svc_opt2_fps_Tanimoto_stdscale_loggers.txt','w')
