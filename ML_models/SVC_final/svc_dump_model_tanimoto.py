...
try:
    ...
    cf = sys.argv[5]   # term added to 'sigmoid' or 'poly' kernels
    d = sys.argv[6]   # degree of kernel function in 'poly'
except:
...
if kernel_type == 'linear':
    model = SVC( C=c, kernel=kernel_type, class_weight='balanced', probability=True )
elif kernel_type == 'tanimoto':
    model = SVC( C=c, kernel=Tanimoto(), class_weight='balanced', probability=True )
elif kernel_type == 'rbf':
    model = SVC( C=c, kernel=kernel_type, gamma=g, class_weight='balanced', probability=True )
elif kernel_type == 'sigmoid':
    model = SVC( C=c, kernel=kernel_type, gamma=g, coef0=cf, class_weight='balanced', probability=True )
